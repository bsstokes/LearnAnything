...
public class Database implements Closeable {
    ...
    public Observable<SqlBrite.Query> getChildren(@NonNull String parentId) {
        ...
        final String queryTemplate = "SELECT * FROM {table} WHERE {parent_id_column} = ?";

        final String table = TableConfig.Children.TABLE;
        final String query = Phrase.from(queryTemplate)
                .put("table", table)
        ...
        return getSqlBrite().createQuery(table, query, parentId);
    }
    ...
    public Observable<SqlBrite.Query> getVideoByReadableId(String readableId) {
        ...
        final String queryTemplate = "SELECT * FROM {table} WHERE {readable_id_column} = ?";

        final String table = TableConfig.Videos.TABLE;
        final String query = Phrase.from(queryTemplate)
                .put("table", table)
                .put("readable_id_column", TableConfig.Videos.COLUMN_READABLE_ID)
                .format().toString();

        logQuery(query);

        return getSqlBrite().createQuery(table, query, readableId);
    }

    public Observable<SqlBrite.Query> getAllVideos() {

        final String queryTemplate = "SELECT * FROM {table}";

        final String table = TableConfig.Videos.TABLE;
        final String query = Phrase.from(queryTemplate).put("table", table).format().toString();
